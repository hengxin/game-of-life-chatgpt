...
paused = False
speed = 500  # Animation speed (interval between frames in milliseconds)
...
def speed_up(event):
    global speed
    speed = max(100, speed - 100)  # Don't go below 100 ms
...
for filename in os.listdir(directory):
    if filename.endswith(".rle"):
        ...
        fig.canvas.mpl_connect('key_press_event', step_forward)
        fig.canvas.mpl_connect('key_press_event', speed_up)
        fig.canvas.mpl_connect('key_press_event', slow_down)

        mat = ax.matshow(grid)
        ani = animation.FuncAnimation(fig, update, frames=15, interval=speed, save_count=50)
